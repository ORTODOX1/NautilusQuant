...
def print_table(report: dict) -> None:
    ...
    rows = [
        ...
        ("Determinism (unique/runs)", f"{t['deterministic_unique']}/{report['runs']}",
         f"{n['deterministic_unique']}/{report['runs']}", True),
    ]
...
def print_attention(rep: dict) -> None:
    ...
    print(f"  KV bytes  FP16={rep['kv_bytes_fp16']:>12,}  "
...
def print_scaling(reports: list) -> None:
    ...
    print(f"{'Model':<14} {'KV FP16':>12} {'KV NQX':>12} "
          f"{'H100':>8} {'NQX':>8} {'savings':>10}")
    print("-" * 72)
    for r in reports:
        savings = f"{r['savings_ratio']:.1f}×"
        print(f"{r['model']:<14} {r['kv_bytes_fp16_human']:>12} "
...
def render_md_report(side, attn, scaling) -> str:
    ...
    lines.append(f"| Energy nJ/vec | {t['energy_nj_per_vec']:.2f} | **{n['energy_nj_per_vec']:.2f}** |")
    lines.append(f"| LUT/PRNG state | {t['state_bytes']} B | **{n['state_bytes']} B** |")
    lines.append(f"| Determinism | {t['deterministic_unique']}/{side['runs']} | **{n['deterministic_unique']}/{side['runs']}** |")
    lines.append("")
...
def main(argv=None) -> int:
    ...
    print("Read demos/pitch.md, demos/why_it_works.md, demos/side_by_side.md "
          "for the deep dive.")
    return 0
